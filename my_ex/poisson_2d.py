...
def pde(x, y):
    ...
    f = k0 ** 2 * cos(k0 * x[:, 0:1]) * cos(k0 * x[:, 1:2])*2
    #f = 6*x[:,0:1]*(1-5*x[:,0:1]**3)*x[:,1:2]**3*(x[:,1:2]**3-1) + 6*x[:,1:2]*(1-5*x[:,1:2]**3)*x[:,0:1]**3*(x[:,0:1]**3-1) 
    return -dy_xx - dy_yy - f


def func(x):
    return np.cos(k0 * x[:, 0:1]) * np.cos(k0 * x[:, 1:2])
    #return x[:,0:1]**3 * x[:,1:2]**3 * (1-x[:,0:1]**3) * (1-x[:,1:2]**3)
